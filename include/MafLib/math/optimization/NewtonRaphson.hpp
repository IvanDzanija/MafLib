...
class NewtonRaphson : public Optimizer<T> {
 public:
  ...
  NewtonRaphson(const std::function<T(T)> &function,
                const std::function<T(T)> &derivative, T start)
 ...
 private:
    ...
    while (max_iterations-- > 0) {
      ...
      if (std::abs(f_prime_x) <= std::numeric_limits<T>::epsilon()) {
        return {
            .solution = x,
            .error = std::abs(f_x),
            .error_message = "Derivative is too small; potential division by zero."};
      }
      ...
      if (rel_error < tolerance) {
        return {.solution = x_new, .error = rel_error, .error_message = std::nullopt};
      }
      x = x_new;
    }
    return {.solution = x,
  ...
  OptimizerResult<T> _secant_solve(T tolerance, int32 max_iterations) {
    return OptimizerResult<T>{.solution = std::numeric_limits<T>::quiet_NaN(),
