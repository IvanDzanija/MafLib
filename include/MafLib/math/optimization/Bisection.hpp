...
class Bisection : public Optimizer<T> {
 public:
   ...
   */
  explicit Bisection(const std::function<T(T)> &function, T lower_bound, T upper_bound)
      : Optimizer<T>(function), _lower_bound(lower_bound), _upper_bound(upper_bound) {}

  /** @brief Get the lower bound of the interval.
   * @return The lower bound.
   */
  [[nodiscard]] T get_lower_bound() const { return _lower_bound; }

  /** @brief Get the upper bound of the interval.
   * @return The upper bound.
   */
  [[nodiscard]] T get_upper_bound() const { return _upper_bound; }

  /**
   * @brief Set the lower bound of the interval.
   * @param lower_bound The lower bound to set.
   */
  void set_lower_bound(T lower_bound) { _lower_bound = lower_bound; }

  /**
   * @brief Set the upper bound of the interval.
   * @param upper_bound The upper bound to set.
   */
  void set_upper_bound(T upper_bound) { _upper_bound = upper_bound; }

  /**
   * @brief Find the root using the Bisection method.
  ...
  [[nodiscard]] OptimizerResult<T> solve(T tolerance = static_cast<T>(1e-7),
    ...
    if (fa * fb > 0) {
      return {.solution = std::numeric_limits<T>::quiet_NaN(),
              ...
              .error_message = "Function has the same sign at the interval endpoints."};
    }
    ...
    while (max_iterations-- > 0) {
      ...
      if (std::abs(fc) < std::numeric_limits<T>::epsilon() || (b - a) / 2 < tolerance) {
        return {.solution = c, .error = std::abs(fc), .error_message = std::nullopt};
      }
